...
const Store = require('../mariadb/mariadb');
...
router.post('/:table/selector', selector);
...
async function selector(req, res, next) {
    try {
        const datos = await Store.selector(req.params.table, req.body);
        response.success(req, res, datos, 200);
    } catch {
        response.error(req, res, { error: true, message: "Error con la base de datos" });
    }
